...
class DPOTrainer(ABC):
    """
    Trainer for Direct Preference Optimization (DPO) training.
    (Code modified to support weighted DPO)
    """
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for batch in self.train_dataloader:
                # ==================== MODIFICATION START: Unpack dictionary batch ====================
                # The dataloader now yields a dictionary, so we access items by key.
                chosen_ids = batch["chosen_input_ids"].squeeze(1).to(torch.cuda.current_device())
                c_mask = batch["chosen_attention_mask"].squeeze(1).to(torch.cuda.current_device())
                reject_ids = batch["rejected_input_ids"].squeeze(1).to(torch.cuda.current_device())
                r_mask = batch["rejected_attention_mask"].squeeze(1).to(torch.cuda.current_device())
                prompt_id_lens = batch["extras"] # 'extras' now holds prompt_id_lens
                loss_weight = batch["loss_weight"].to(torch.cuda.current_device())
                # ===================== MODIFICATION END: Unpack dictionary batch =====================
                ...
                if weight_sum > 1e-6:
                    weighted_loss = (preference_loss * loss_weight).sum() / weight_sum
                    # 计算被选中的数据的准确率
                    active_acc = ((chosen_reward > reject_reward).float() * loss_weight).sum() / weight_sum
                else:
                    weighted_loss = preference_loss.mean() * 0.0
                    active_acc = torch.tensor(0.0).to(torch.cuda.current_device())

                # # ==================== MODIFICATION START: Apply loss weighting ====================
                # # `preference_loss` is a tensor of shape (batch_size,).
                # # We multiply it element-wise by the weights and then take the mean.
                # # weighted_loss = (preference_loss * loss_weight).mean()
                # weight_sum = loss_weight.sum()
                # if weight_sum > 0:
                #     weighted_loss = (preference_loss * loss_weight).sum() / weight_sum
                # else:
                #     # 如果整个 batch 权重都是 0，则 loss 设为 0（这种情况在 Top 30% 策略下可能发生）
                #     weighted_loss = preference_loss.sum() * 0
                # # ===================== MODIFICATION END: Apply loss weighting =====================
                ...
                # The final loss for backpropagation is the weighted DPO loss plus other auxiliary losses.
                loss = weighted_loss + aux_loss * self.args.aux_loss_coef + nll_loss * self.args.nll_loss_coef
                ...
                # --- 改进日志记录 ---
                acc = (chosen_reward > reject_reward).float().mean().item()
                acc_sum += active_acc.item() # 注意：这里建议累加 active_acc
                loss_sum += weighted_loss.item()

                # acc = (chosen_reward > reject_reward).float().mean().item()
                # acc_sum += acc
                # # For logging, we can log the unweighted preference loss mean.
                # loss_sum += preference_loss.mean().item()

                logs_dict = {
                    "loss": preference_loss.mean().item(),
                    "weighted_loss": weighted_loss.item(),
                    "acc": acc,                # 全量准确率
                    "active_acc": active_acc.item(), # 真正参与训练的数据的准确率
                    "chosen_reward": chosen_reward.mean().item(),
                    "reject_reward": reject_reward.mean().item(),
                    "weight_sum": weight_sum.item(), # 监控每批有多少有效数据
                    "lr": self.scheduler.get_last_lr()[0],
                    # "loss": preference_loss.mean().item(), # Log unweighted loss
                    # "weighted_loss": weighted_loss.item(), # Log weighted loss
                    # "acc": acc,
                    # "chosen_reward": chosen_reward.mean().item(),
                    # "reject_reward": reject_reward.mean().item(),
                    # "lr": self.scheduler.get_last_lr()[0],
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        if global_step % args.logging_steps == 0:
            if self._wandb is not None and self.strategy.is_rank_0():
                ...
                for k, v in logs_dict.items():
                    self._tensorboard.add_scalar(f"train/{k}", v, global_step)
            if self.strategy.is_rank_0() and hasattr(self.args, "use_swanlab") and self.args.use_swanlab:
                swanlab_logs = {"train/" + k: v for k, v in logs_dict.items()}
                swanlab.log(swanlab_logs, step=global_step)
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for batch in eval_dataloader:
                # ==================== MODIFICATION START: Unpack dictionary batch for eval ====================
                chosen_ids = batch["chosen_input_ids"].squeeze(1).to(torch.cuda.current_device())
                c_mask = batch["chosen_attention_mask"].squeeze(1).to(torch.cuda.current_device())
                reject_ids = batch["rejected_input_ids"].squeeze(1).to(torch.cuda.current_device())
                r_mask = batch["rejected_attention_mask"].squeeze(1).to(torch.cuda.current_device())
                prompt_id_lens = batch["extras"]
                # loss_weight is not used in evaluation, but the key exists.
                # ===================== MODIFICATION END: Unpack dictionary batch for eval =====================
                ...
                acc_sum += (chosen_reward > reject_reward).float().mean().item()
                loss_sum += loss.mean().item() # Use mean() for robustness
                times += 1
                step_bar.update()
            ...
            if self.strategy.is_rank_0():
                if self._wandb is not None:
                    ...
                    for k, v in logs.items():
                        self._tensorboard.add_scalar(f"eval/{k}", v, steps)
                if hasattr(self.args, "use_swanlab") and self.args.use_swanlab:
                    swanlab_logs = {"eval/" + k: v for k, v in logs.items()}
                    swanlab.log(swanlab_logs, step=steps)
        self.model.train()
